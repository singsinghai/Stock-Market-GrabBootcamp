...
def updateValuation(symbol, year, quarter):
    ...
    result = [
        ...
        str(sum(Val_List['book_value'])/len(Val_List['book_value'])),
        str(calculate_upsize(ref_valuation['book_value'], (sum(Val_List['book_value'])/len(Val_List['book_value'])))),
        
        str(max(Val_List['earnings_per_share'])),
        str(min(Val_List['earnings_per_share'])),
        str(sum(Val_List['earnings_per_share'])/len(Val_List['earnings_per_share'])),
        str(ref_valuation['earnings_per_share'] / (sum(Val_List['earnings_per_share'])/len(Val_List['earnings_per_share']))),
        
        str(max(Val_List['enterprise_value'])),
        str(min(Val_List['enterprise_value'])),
        str(sum(Val_List['enterprise_value'])/len(Val_List['enterprise_value'])),
        str(calculate_upsize(ref_valuation['enterprise_value'], (sum(Val_List['enterprise_value'])/len(Val_List['enterprise_value'])))),
        
        str(max(Val_List['ev_over_ebit'])),
        str(min(Val_List['ev_over_ebit'])),
        str(sum(Val_List['ev_over_ebit'])/len(Val_List['ev_over_ebit'])),
        str(calculate_upsize(ref_valuation['ev_over_ebit'], (sum(Val_List['ev_over_ebit'])/len(Val_List['ev_over_ebit'])))),
        
        str(max(Val_List['ev_over_ebitda'])),
        str(min(Val_List['ev_over_ebitda'])),
        str(sum(Val_List['ev_over_ebitda'])/len(Val_List['ev_over_ebitda'])),
        str(calculate_upsize(ref_valuation['ev_over_ebitda'], (sum(Val_List['ev_over_ebitda'])/len(Val_List['ev_over_ebitda'])))),
        
        str(max(Val_List['ev_sales'])),
        str(min(Val_List['ev_sales'])),
        str(sum(Val_List['ev_sales'])/len(Val_List['ev_sales'])),
        str(calculate_upsize(ref_valuation['ev_sales'], (sum(Val_List['ev_sales'])/len(Val_List['ev_sales'])))),
        
        str(max(Val_List['price_to_book'])),
        str(min(Val_List['price_to_book'])),
        str(sum(Val_List['price_to_book'])/len(Val_List['price_to_book'])),
        str(calculate_upsize(ref_valuation['price_to_book'], (sum(Val_List['price_to_book'])/len(Val_List['price_to_book'])))),
        
        str(max(Val_List['price_earnings'])),
        str(min(Val_List['price_earnings'])),
        str(sum(Val_List['price_earnings'])/len(Val_List['price_earnings'])),
        str(calculate_upsize(ref_valuation['price_earnings'], (sum(Val_List['price_earnings'])/len(Val_List['price_earnings'])))),
        
        str(max(Val_List['price_to_sales'])),
        str(min(Val_List['price_to_sales'])),
        str(sum(Val_List['price_to_sales'])/len(Val_List['price_to_sales'])),
        str(calculate_upsize(ref_valuation['price_to_sales'], (sum(Val_List['price_to_sales'])/len(Val_List['price_to_sales'])))),
        
        str(max(Val_List['market_cap'])),
        str(min(Val_List['market_cap'])),
        str(sum(Val_List['market_cap'])/len(Val_List['market_cap'])),
        str(calculate_upsize(ref_valuation['market_cap'], (sum(Val_List['market_cap'])/len(Val_List['market_cap']))))
    ]
    return result
def populate_stockvaluation(year, quarter, upsert=False, verbose=False):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=60) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_stock):
            ...
            temp_sql = sql + ', '.join(['(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)'])
            if upsert:
